...
void Mesh::processMesh(const aiMesh *mesh) {
  ...
  this->mesh.nIndices = mesh->mNumFaces * 3;
...
// still here for the unit cube, which needs to be loaded without being an entity
...
#ifdef DEBUG
  std::cout << "Loaded " << scene->mNumMeshes << " mesh(es) [" << n_vertices
...
void Mesh::draw() {
  ...
  glDrawElements(GL_TRIANGLES, static_cast<unsigned int>(mesh.nIndices), GL_UNSIGNED_INT, 0);
